...
void Process::Pid(int id) { pid_ = id; }

void Process::User(std::string user) { user_ = user; }

void Process::Command(std::string command) { command_ = command; }

void Process::CpuUtilization(float cpu_utilisation) {
  cpu_utilisation_ = cpu_utilisation;
}
...
bool Process::operator<(Process const& a) const {
  return cpu_utilisation_ < a.cpu_utilisation_;
}